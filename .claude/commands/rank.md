...
3. **Location & work-authorization veto** (see `04-job-evaluation.md` dimension 4 — the candidate is a **Russian citizen in Dubai**, goal **start fast**): `FAIL` is a **work-authorization wall**, not "requires relocation" — explicit no-sponsorship / must-have-existing-work-auth, region-locked remote needing local work-auth (US/EU-only) with no sponsorship/EOR, citizens-/nationals-only, active clearance, or a native/fluent non-English/Russian language requirement. `FAIL` excludes the job no matter the score - list it separately with the reason. `FLAG` (silent on sponsorship, hybrid/onsite of uncertain support, or heavy travel) stays in the ranking with a visible ⚠ "confirm sponsorship" marker. Fully-remote (global) and UAE/MENA roles are `PASS` **and** carry the start-fast boost (Career Alignment); relocation-with-sponsorship roles are `PASS` without the boost, so they rank below remote.
